    ...
    <div class="max">
        <div class="box">
            <div class="mox"> 
                ...
                <div class="dfa plplo">
                   <div> <div class="tubiao"><FormOutlined /></div><div>评论({{omg}})</div></div>
                ...
                <div  v-for="(item,index) in msg" :key="index">
                    <div class="content" v-for="(item1,index1) in item.comments" :key="index1">
                        <div class="nbsp">
                            <div class="divss"><img style="width:100%;" :src="`http://157.122.54.189:9095${item.account.defaultAvatar}`" alt=""></div>
                            <div>{{item.account.nickname}}</div>
                            <div>{{item1.created_at}}</div>
                        </div>
...
onMounted(()=>{
    ...
    api.getpostwen({id:Number(data.id)}).then(res=>{
        ...
        res.data.map((item:any)=>{
            data.omg=item.comments.length
            item.comments.map((item1:any)=>{
                item1.created_at=dayjs(item1.created_at).format('YYYY-MM-DD HH:mm:ss')
            })
        })
        data.msg=res.data
        console.log(res)
