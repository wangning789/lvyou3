    ...
    <div class="box dfa">
      <div>
        ...
        <a-card class="max" v-if="num===0">
          <a-form ref="ruleForm" :model="form" :rules="rules"  @finish="login">
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    let login=():void=>{
      console.log('123',data.form.passwrod)
      // api.getlogin({username:data.form.name,password:data.form.passwrod}).then((res:any)=>{
      //   console.log(res)
      // }).catch((err:any)=>{
      //   console.log(err)
      // })
    ...
    let submission=(values:any):void=>{
      api.postnox({
        username:data.vform.ipone,
        nickname:data.vform.usname,
        captcha:data.vform.verification,
        password:data.vform.aspassword
      }).then((res:any)=>{
        if(res.token){
