          ...
          <a class="ant-dropdown-link" @click="e => e.preventDefault()">
            <div class="xianzhi">
              ...
              <div v-if="levervalue.length===1">
                <div v-for="item1 in levervalue" :key="item1">{{item1}}</div>
              </div>
          ...
          <template v-slot:overlay>
            <a-menu>
              ...
              <a-menu-item v-for="(item,index) in plainOptions" :key="index">
                <a-checkbox @change="onChange(item)">{{item.name}}</a-checkbox>
              </a-menu-item>
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    let onChange = (checkedValue: any): void => {
      console.log(data.plainOptions);
      console.log('123',checkedValue)
       
       if(data.levervalue.length<1){
           data.levervalue.push(checkedValue)
       }else{
           let arr=data.levervalue.filter(item=>{
