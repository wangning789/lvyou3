<template>
  <div style=" display: flex;">
     ...
     <div class="lener">
      ...
      <div>
        <a-dropdown>
          <a class="ant-dropdown-link" @click="e => e.preventDefault()">
            <div class="xianzhi">
              <div v-if="levervalue3.length<1">不限</div>
              <div v-if="levervalue3.length===1">
                <div v-for="item1 in levervalue3" :key="item1">{{item1.name}}</div>
              </div>
              <div v-if="levervalue3.length>1">已选{{levervalue3.length}}项</div>
              <div>
                <DownOutlined />
              </div>
            </div>
          ...
          <template v-slot:overlay>
            <a-menu>
                <a-checkbox-group v-model:value="levervalue3" >
              <a-menu-item v-for="(item,index) in assets" :key="index">
                <a-checkbox :value="item" @change="streeChange">{{item.name}}</a-checkbox>
              </a-menu-item>
              </a-checkbox-group>
            </a-menu>
          </template>
    ...
    <div class="lener">
      ...
      <div>
        <a-dropdown >
          <a class="ant-dropdown-link" @click="e => e.preventDefault()" @getPopupContainer='() => document.body'>
            <div class="xianzhi">
              <div v-if="levervalue4.length<1">不限</div>
              <div v-if="levervalue4.length===1">
                <div v-for="item1 in levervalue4" :key="item1">{{item1.name}}</div>
              </div>
              <div v-if="levervalue4.length>1">已选{{levervalue4.length}}项</div>
              <div>
                <DownOutlined />
              </div>
            </div>
          ...
          <template v-slot:overlay>
            <a-menu>
                <a-checkbox-group v-model:value="levervalue4" >
              <a-menu-item v-for="(item,index) in brands" :key="index">
                <a-checkbox :value="item" @change="foruChange">{{item.name}}</a-checkbox>
              </a-menu-item>
              </a-checkbox-group>
            </a-menu>
          </template>
...
interface Data {
  ...
  levervalue3:Array<object>
  levervalue4:Array<object>
  assets:Array<object>;
  brands:Array<object>
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    let onChange = (checkedValue: any): void => {
     ...
     let streeChange = (checkedValue: any): void => {
       setTimeout(function(){
           console.log(data.levervalue3)
       },1000)
    };

     let foruChange = (checkedValue: any): void => {
       setTimeout(function(){
           console.log(data.levervalue4)
       },1000)
    };
