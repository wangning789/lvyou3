  ...
  <div>
    <div class="max">
      <div class="box">
        ...
        <div>
          <a-range-picker size="large" show-time v-model:value="oneday" :disabled-date="disabledDate"  :placeholder="['开始日期','结束日期']"/>
        </div>
        <div> 
        ...
        <div><div id="container" class="map"></div></div>
      </div>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    onMounted(()=>{
      ...
      api.gethoel({name:'成都'}).then((res:any)=>{
        console.log(res)
      }).catch((err:any)=>{
        console.log(err)
      })
